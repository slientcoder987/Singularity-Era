...
成为 AI 行业的霸主。通过训练更强的模型、发布更优质的 API 服务、扩张全球市场，最终在竞争中胜出。

**胜利条件**（任选其一）：
- 训练出参数规模达到 1T 的模型
- 在全球 33 个地区中的 20 个以上发布 API
- 收购所有 6 家竞争对手
- 累计收入达到 $1B
...
1. **基础设施维护**：硬件磨损、电力消耗、维护费用
2. **员工管理**：薪资发放、忠诚度变化、绩效评估
3. **研发进展**：科技研究、模型训练、数据采集
4. **运营收益**：API 服务收入、用户增长
5. **竞争对手行动**：融资、训练、发布模型、市场扩张
6. **风险事件**：法律纠纷、信任危机、安全事件
...
**资源配置**：
- 招聘员工（5 种角色，需要平衡团队）
- 采购算力（7 种硬件，考虑性价比和交付时间）
- 建设基础设施（数据中心、集群、节点）

**研发管理**：
- 启动模型训练项目（选择架构、数据集、算力分配）
- 管理训练进度（监控损失曲线、处理异常）
- 评估模型能力（多维度评测）
