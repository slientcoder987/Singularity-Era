...
for (let day = 1; day <= TARGET_DAYS; day++) {
  ...
  if (day >= 4 && day <= 120) {
    ensureResearchers(4);
    // P0 基础技术链
    tryExperiment('flash_attention', 7);
    tryExperiment('rmsnorm', 7);
    tryExperiment('pre_ln', 7);
    tryExperiment('zero_1', 7);
    tryExperiment('rope', 7);
    tryExperiment('swiglu', 7);
    tryExperiment('data_cleaning_v1', 7);
    if (hasTech('flash_attention')) tryExperiment('flash_attention_2', 7);
    if (hasTech('pre_ln')) { tryExperiment('stable_training', 7); tryExperiment('gradient_clipping', 7); }
    if (hasTech('data_cleaning_v1')) tryExperiment('data_deduplication', 7);
    if (hasTech('rmsnorm')) tryExperiment('quantization', 7);
  }
  ...
  if (day === 40) { once('install-40', () => P.installAllCards()); }
  ...
  if (day >= 65 && day <= 250) {
    ensureResearchers(6);
    if (hasTech('flash_attention_2')) tryExperiment('pipeline_parallel', 7);
    if (hasTech('zero_1') && hasTech('pipeline_parallel')) tryExperiment('tensor_parallel', 7);
    if (hasTech('pipeline_parallel')) tryExperiment('context_parallel', 7);
    // 架构技术
    tryExperiment('moe', 7);
    if (hasTech('moe') && hasTech('pipeline_parallel')) tryExperiment('expert_parallel', 7);
    // 后训练技术
    if (hasTech('pretraining')) tryExperiment('sft', 7);
    if (hasTech('sft')) { tryExperiment('rlhf', 7); tryExperiment('cot_training', 7); }
    if (hasTech('rlhf')) tryExperiment('dpo', 7);
    if (hasTech('rope') && hasTech('flash_attention_2')) tryExperiment('long_context_training', 13);
  }
  ...
  if (day === 100) { once('install-100', () => P.installAllCards()); }
  ...
  if (day === 210) { once('install-210', () => P.installAllCards()); }
  ...
  if (day === 410) { once('install-410', () => P.installAllCards()); }
